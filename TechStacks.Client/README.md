...
The production build uses Next.js static export to generate a fully static website that can be served by the C# backend.
...
The app is configured with `output: 'export'` in production mode, which:
- Generates static HTML files for all routes
- Creates `index.html` in the root of `./dist`
- Outputs all assets to `./dist/_next/static/`
- Supports client-side routing for dynamic routes

### Dynamic Routes

Dynamic routes (`/tech/[slug]`, `/stacks/[slug]`, etc.) use placeholder pages:
- A single placeholder page is generated at build time (e.g., `/tech/_placeholder/`)
- Actual content is loaded client-side via API calls
- The C# backend's `MapFallbackToFile("index.html")` handles routing for non-existent paths

### Deployment

The build script copies the static export to the C# project's wwwroot:

```bash
npm run build
# Runs: next build && cp -r dist/* ../TechStacks/wwwroot/
```

The C# backend (`TechStacks/Program.cs`) serves the static files with:
